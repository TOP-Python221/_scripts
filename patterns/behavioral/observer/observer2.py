...
class Patient(ABC):
    """Интерфейс для наблюдаемых объектов."""
    ...
    def update_monitors(self, patient: 'Patient'):
...
class COVIDPatient(Patient):
    ...
    def set_parameter(self, parameter: str, value):
        if parameter in self.__parameters:
            ...
            self.update_monitors(patient=self)
...
class Monitor(ABC):
    ...
    def update(self, subject: Patient):
        """Анализирует состояние наблюдаемого объекта."""
...
class Thermometer(Monitor):
    ...
    def update(self, subject: Patient):
        ...
        if 36.5 <= temperature <= 37.0:
            self.info_message(message)
        elif 37.0 < temperature < 38.3:
